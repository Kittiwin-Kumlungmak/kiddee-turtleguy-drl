...
class DRLEnvironment(Node):
    def __init__(self):
        ...
        self.mudang_goaldist = math.dist((0,0), (self.mudang_destx,self.mudang_desty))
    ...
    *******************************************************************************"""
    ...
    def scan_callback(self, msg):
        ...
        for i in range(NUM_SCAN_SAMPLES):
                self.scan_ranges[i] = numpy.clip(float(num_msg[i]) / MAX_LIDAR_VALUE, 0, 1)
    ...
    def step_comm_callback(self, request, response):
        ...
        if self.local_step % 200 == 0:
            print(f"Rtot: {response.reward:.3f}, GD: {self.mudang_goaldist:.3f}, GA: {math.degrees(self.goal_angle):.3f}° \
